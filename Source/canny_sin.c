...
void comprobar(float suma[]){
	...
	file=fopen("arreglo.txt","r");

	fgets(buff,250,file);
	buff[strlen(buff)-2] = '\0';
	while((!(feof(file)))&&(i<valor)){	
		sprintf(comp,"%.4f",suma[i]);	
		if(strcmp(comp,buff)!=0){error++;printf("fallo el valor de i %d el valor del arreglo es %f y el valor tiene que ser %s \n",i,suma[i],buff);}		
		i++;
		fgets(buff,250,file);	
		buff[strlen(buff)-2] = '\0';	
	}
	fclose(file);
	printf("La cantidad de errores es %d \n",error);
}

void crear(float suma[]){
	int i;
	
	FILE *file;

	file=fopen("arreglo.txt","w");
