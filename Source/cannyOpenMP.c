...
int main(int argc, char *argv[])
{
   ...
   ****************************************************************************/
   ...
   if(dirfilename != NULL){
	...
	tini_local = omp_get_wtime ();
    canny(image, rows, cols, sigma, tlow, thigh, &edge, dirfilename);
	tfin_local = omp_get_wtime ();
	printf ("-----------------------------------------\nDemoro:\"Canny\": %f \n", tfin_local - tini_local);

   /****************************************************************************
   * Write out the edge image to a file.
   ****************************************************************************/
   sprintf(outfilename, "%s_s_%3.2f_l_%3.2f_h_%3.2f.pgm", "openmp",
      sigma, tlow, thigh);
        ...
        short int **magnitude)
{
   ...
   ****************************************************************************/
   ...
   #pragma omp parallel for private(c,sq1,sq2)
   for(r=0;r<rows;r++){
      for(c=(cols*r);c<(cols*(r+1));c++){
         sq1 = (int)delta_x[c] * (int)delta_x[c];
         sq2 = (int)delta_y[c] * (int)delta_y[c];
         (*magnitude)[c] = (short)(0.5 + sqrt((float)sq1 + (float)sq2));
      }
   }
}
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   tini_local = omp_get_wtime ();
   for(r=0;r<rows;r++){
   	  #pragma omp parallel for private(cc,sum,dot)
      for(c=0;c<cols;c++){
         dot = 0.0;
         sum = 0.0;
   ...
   ****************************************************************************/
   ...
   for(c=0;c<cols;c++){
   	  #pragma omp parallel for private(rr,sum,dot)
      for(r=0;r<rows;r++){
         sum = 0.0;
         dot = 0.0;
...
void make_gaussian_kernel(float sigma, float **kernel, int *windowsize)
{
	...
	*windowsize = 1 + 2 * ceil(2.5 * sigma);
	...
	//#pragma omp parallel for private (i) reduction (/:*kernel)
	...
	float tlow, float thigh, unsigned char *edge)
{
   ...
   ****************************************************************************/
    ...
    for(r=0;r<rows;r++){
     for(c=(cols*r);c<(cols*(r+1));c++){
	 	if(nms[c] == POSSIBLE_EDGE) edge[c] = POSSIBLE_EDGE;
	 	else edge[c] = NOEDGE;
      	}
   }
   ...
   ****************************************************************************/
   ...
   #pragma omp parallel for private(c) //tenia schedule (dynamic) y andaba mal
   for(r=0;r<rows;r++){
      for(c=(cols*r);c<(cols*(r+1));c++){
	 	if(edge[c] == POSSIBLE_EDGE) hist[mag[c]]++;
      }
   }
   /****************************************************************************
   ...
   ****************************************************************************/
   ...
   for(r=0;r<rows;r++){
      for(c=(cols*r);c<(cols*(r+1));c++){
	 		if((edge[c] == POSSIBLE_EDGE) && (mag[c] >= highthreshold)){
            	edge[c] = EDGE;
            	follow_edges((edge+c), (mag+c), lowthreshold, cols);
	 		}
      	}
   	}
   /****************************************************************************
   ...
   ****************************************************************************/
   #pragma omp parallel for schedule(dynamic) private(c)
   for(r=0;r<rows;r++){
      for(c=(cols*r);c<(cols*(r+1));c++) if(edge[c] != EDGE) edge[c] = NOEDGE;
   }
    ...
    unsigned char *result)
{
    ...
    double tiniNM, tfinNM;
    tiniNM = omp_get_wtime ();

   /****************************************************************************
   * Zero the edges of the result image.
   ****************************************************************************/

    double tini44, tfin44;
    // los siguientes 2 for no duran nada, no sirve paralelizar
   ...
   ****************************************************************************/
   tini44 = omp_get_wtime ();
   magrowptr=mag+1;
   gxrowptr=gradx+1;
   gyrowptr=grady+1;
   resultrowptr=result+1;
   #pragma omp parallel for private(colcount, magptr, gxptr,gyptr,resultptr,m00,xperp,gx,yperp,gy,z1,z2,mag1,mag2)
   for(rowcount=1; rowcount<nrows-2;rowcount++){
      ...
      for(colcount=1;colcount<ncols-2;colcount++,magptr++,gxptr++,gyptr++,resultptr++){
         m00 = *magptr;
         if(m00 == 0){
    ...
    tfin44 = omp_get_wtime ();
    printf ("-----------------------------------------\nDemoro: \"forgrande_non_max\": %f \n", tfin44 - tini44);

    tfinNM = omp_get_wtime ();
    printf ("-----------------------------------------\nDemoro: \"non_max_supp\": %f \n", tfinNM - tiniNM);
}
//<------------------------- end hysteresis.c ------------------------->
