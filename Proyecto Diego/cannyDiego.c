...
/* se agrega la declaracion del metodo non_max_supp */
...
void magnitude_x_y(short int *delta_x, short int *delta_y, int rows, int cols,
        short int **magnitude)
{
	double tini2, tfin2, tini3, tfin3;			/* para medicion de tiempos */
	int sharedPos;								/* variable pos compartida */
   int r, c, pos, sq1, sq2;
   ...
   ****************************************************************************/
   ...
   //tini3 = omp_get_wtime ();
   /* similar al metodo apply_hysteresis */
   pos=0;
   sharedPos = - cols;
   #pragma omp parallel for shared (magnitude, delta_x, delta_y) private (c, pos, sq1, sq2)
   for(r=0;r<rows;r++){
	  #pragma omp critical
	  {
		  sharedPos += cols;
   ...
   //tfin3 = omp_get_wtime ();
   //rintf (">>>>>>>>>>>>>>Allocate an image to store the magnitude of the gradient demoro: %f\n", tfin3 - tini3);
   tfin2 = omp_get_wtime ();
        ...
        short int **delta_x, short int **delta_y)
{
   ...
   ****************************************************************************/
   ...
   //tini3 = omp_get_wtime ();
   /* cada hilo asigna el valor de pos dependiendo de la iteracion en el bucle
    * exterior en la cual se encuentre, debido a que cada hilo anterior
    * incremento el valor de pos en cols veces */
   #pragma omp parallel for shared (delta_x) private (c, pos)
   for(r=0;r<rows;r++){
      pos = r * cols;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>Computing the X-direction derivative demoro: %f\n", tfin3 - tini3);
   ...
   ****************************************************************************/
   ...
   #pragma omp parallel for shared (delta_y) private (r, pos)
   for(c=0;c<cols;c++){
      pos = c;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>Computing the Y-direction derivative demoro: %f\n", tfin3 - tini3);
   tfin2 = omp_get_wtime ();
...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma,
        short int **smoothedim)
{
	double tini2, tfin2, tini3, tfin3;		/* variables para medir tiempos */
   ...
   ****************************************************************************/
   if(VERBOSE) printf("   Bluring the image in the X-direction.\n");
	//tini3 = omp_get_wtime ();
	/* interesa que tanto c como cc sean variables privadas para que cada hilo
	 * haga su propio bucle, y dot y sum para sus propios calculos;
	 * no hay necesidad de sincronizacion en la variable tempim ya que cada
	 * hilo accede a una posicion distinta en el arreglo */
	#pragma omp parallel for shared (tempim) private (c, cc, dot, sum)
	for(r=0;r<rows;r++){
		for(c=0;c<cols;c++){
			dot = 0.0;
			sum = 0.0;
	...
	//tfin3 = omp_get_wtime ();
	//printf (">>>>>>>>>>>>>>Bluring the image in the X-direction demoro: %f\n", tfin3 - tini3);
   ...
   ****************************************************************************/
   if(VERBOSE) printf("   Bluring the image in the Y-direction.\n");
	//tini3 = omp_get_wtime ();
	/* similar al caso anterior, pero con r y rr;
	 * y *smoothedim tambien es accedido por cada hilo en una direccion
	 * distinta */
	#pragma omp parallel for shared (smoothedim) private (r, rr, dot, sum)
   for(c=0;c<cols;c++){
      for(r=0;r<rows;r++){
         sum = 0.0;
         dot = 0.0;
	...
	//tfin3 = omp_get_wtime ();
	//printf (">>>>>>>>>>>>>>Bluring the image in the Y-direction demoro: %f\n", tfin3 - tini3);
	...
	float tlow, float thigh, unsigned char *edge)
{
   ...
   ****************************************************************************/
   //de los tres bucles for, el primero es el unico a tener en cuenta debido
   //al tiempo de computo, se modifica el codigo para solucionar la dependencia
   //de datos que hay con la variable pos
   //tini3 = omp_get_wtime ();
   /* pos es la variable privada de cada hilo para saber donde acceder, sharedPos
    * es la misma variable pero compartida, donde cada hilo incrementa el valor
    * de sharedPos en cols, que es la cantidad que incremento el hilo anterior,
    * y luego se asigna dicho valor incrementado como su variable pos;
    * se inicializa sharedPos como -cols debido a que primero se incrementa el
    * valor y luego se asigna, por lo que es deseado que el 1er hilo arranque
    * con el valor de pos = 0 */
   pos=0;
   sharedPos = - cols;
   #pragma omp parallel for shared (edge, nms) private (pos, c)
   for(r=0;r<rows;r++){
		#pragma omp critical
		{
			sharedPos += cols;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>La region paralela 1 demoro: %f\n", tfin3 - tini3);
   ...
   ****************************************************************************/
   //similar al region anterior, se modifica el codigo para solucionar la
   //dependencia de datos que hay con la variable pos
   //tini3 = omp_get_wtime ();
   pos=0;
   sharedPos = - cols;
   for(r=0;r<32768;r++) hist[r] = 0;
   #pragma omp parallel for shared (edge, hist, mag) private (pos, c)
   for(r=0;r<rows;r++){
		#pragma omp critical
		{
			sharedPos += cols;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>La region paralela 2 demoro: %f\n", tfin3 - tini3);

   /****************************************************************************
   * Compute the number of pixels that passed the nonmaximal suppression.
   ****************************************************************************/
   //se ignora esta region debido al insignificante tiempo de computo
   ...
   ****************************************************************************/
   //esta region se ignora debido al insignificante tiempo de computo
   r = 1;
   numedges = hist[1];
   ...
   ****************************************************************************/
   //similar a las regiones 1 y 2, se modifica el codigo para solucionar la
   //dependencia de datos que hay con la variable pos
   /* cada hilo a su vez realiza la llamada a la funcion recursiva follow_edges */
   //tini3 = omp_get_wtime ();
   sharedPos = - cols;
   #pragma omp parallel for shared (edge, mag) private (pos, c)
   for(r=0;r<rows;r++){
		#pragma omp critical
		{
			sharedPos += cols;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>La region paralela 4 demoro: %f\n", tfin3 - tini3);

   /****************************************************************************
   * Set all the remaining possible edges to non-edges.
   ****************************************************************************/
   //similar a las regiones 1, 2 y 4, se modifica el codigo para solucionar
   //la dependencia de datos que hay con la variable pos
   //tini3 = omp_get_wtime ();
   sharedPos = - cols;
   #pragma omp parallel for shared (edge) private (pos, c)
   for(r=0;r<rows;r++){
		#pragma omp critical
		{
			sharedPos += cols;
   ...
   //tfin3 = omp_get_wtime ();
   //printf (">>>>>>>>>>>>>>La region paralela 5 demoro: %f\n", tfin3 - tini3);
   tfin2 = omp_get_wtime ();
...
void non_max_supp(short *mag, short *gradx, short *grady, int nrows, int ncols,
    unsigned char *result) 
{
	double tini2, tfin2, tini3, tfin3;		/* para medir los tiempos */
   ...
   ****************************************************************************/
   /* se inicializa cada puntero sin sumar ncols */
   magrowptr=mag+1;
   gxrowptr=gradx+1;
   gyrowptr=grady+1;
   resultrowptr=result+1;
   //tini3 = omp_get_wtime ();
   /* las variables privadas son aquellas que se incrementan en el bucle
    * exterior, el cual es compartido por los hilos;
    * las variables internas son los punteros que referencian a las
    * variables externas, como asi tambien los de calculo como z1, z2
    * mag1, etc */
   ...
   for (rowcount=1; rowcount<nrows-2; rowcount++) { 
	   /* cada hilo suma los punteros compartidos la cantidad de ncols,
	    * que es la cantidad que suma cada hilo anterior debido al bucle 
	    * interno, y se asigna los suyos con dicho valor incrementado */
	   #pragma omp critical
	   {
			magrowptr+=ncols;
			gyrowptr+=ncols;
    ...
    //tfin3 = omp_get_wtime ();
    //printf (">>>>>>>>>>>>>>Suppress non-maximum points demoro: %f\n", tfin3 - tini3);
    tfin2 = omp_get_wtime ();
